<script>
  import { isDef } from 'UI/src/utils/shared';
  import scrollIntoView from 'UI/src/utils/scroll-into-view';
  import { generateId } from 'UI/src/utils/util';
  ...
  export default {
    ...
    methods: {
      ...
      activeItem(item, menuIndex,event) {
        ...
        // console.log(this.expandTrigger);
        if (this.changeOnSelect && this.expandTrigger==="click") {
          this.$emit('pick', this.activeValue.slice(), false);
        } else if (!this.changeOnSelect && this.expandTrigger==="hover" && event==="mousedown") {
         this.$emit('pick', this.activeValue.slice(), true);
        }else if (!this.changeOnSelect && this.expandTrigger==="hover") {
        ...
        const items = this._l(menu, item => {
          ...
          if (!item.disabled) {
            ...
            if (item.children) {
              ...
              events.on['mousedown'] = () => {
                this.clicking = true;
                 this.activeItem(item, menuIndex,'mousedown');
              };
